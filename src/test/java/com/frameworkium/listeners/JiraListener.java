...
public class JiraListener implements ITestListener {
	...
	public void onTestStart(ITestResult result) {
		...
		{        	
			String comment = String.format("Starting %s.%s", result.getTestClass()
					.getName(), result.getMethod().getMethodName());    	

			new Execution(JIRA_RESULT_VERSION.getValue(), getIssueAnnotation(result))
	...
	public void onTestSuccess(ITestResult result) {
		...
		if(loggingParamsProvided(result))
		{        	
			String comment = String.format("%s.%s Passed in %s seconds", result.getTestClass()
					.getName(), result.getMethod().getMethodName(),
	...
	public void onTestFailure(ITestResult result) {
		...
		{       
			...
			String comment = String.format("%s.%s Failed in %s seconds", result.getTestClass()
					.getName(), result.getMethod().getMethodName(),
					((result.getEndMillis() - result.getStartMillis())/1000));
	...
	public void onTestSkipped(ITestResult result) {
		...
		if(loggingParamsProvided(result))
		{        	
			String comment = String.format("%s.%s Skipped in %s seconds", result.getTestClass()
					.getName(), result.getMethod().getMethodName(),
					((result.getEndMillis() - result.getStartMillis())/1000));
	...
	private Boolean loggingParamsProvided(ITestResult result)
	{
		if(	JIRA_URL.isSpecified() &&
				JIRA_RESULT_VERSION.isSpecified() &&
	...
	private String getIssueAnnotation(ITestResult result)
	{
		try
		{ 
			return result.getMethod().getConstructorOrMethod()
					.getMethod().getAnnotation(Issue.class).value();
		}
