...
import static com.bootstrapium.config.DriverBinaryContext.binaryFor;
import static com.bootstrapium.config.DriverBinaryContext.binaryPath;
import static com.bootstrapium.config.DriverType.*;
import static com.bootstrapium.config.OperatingSystem.*;
import static com.bootstrapium.config.SystemArchitecture.ARCHITECTURE_32_BIT;
import static com.bootstrapium.config.SystemArchitecture.ARCHITECTURE_64_BIT;
    ...
    private static final Map<DriverBinaryContext, String> binaryLocation = new HashMap<DriverBinaryContext, String>() {
        {
            put(binaryFor(CHROME, WINDOWS, ARCHITECTURE_64_BIT),
                    binaryPath("/windows/googlechrome/64bit/2.10/chromedriver.exe"));
            put(binaryFor(CHROME, MAC, ARCHITECTURE_64_BIT),
                    binaryPath("/osx/googlechrome/64bit/2.10/chromedriver"));
            put(binaryFor(CHROME, LINUX, ARCHITECTURE_64_BIT),
                    binaryPath("/linux/googlechrome/64bit/2.10/chromedriver"));
            put(binaryFor(CHROME, WINDOWS, ARCHITECTURE_32_BIT),
                    binaryPath("/windows/googlechrome/32bit/2.10/chromedriver.exe"));
            put(binaryFor(CHROME, MAC, ARCHITECTURE_32_BIT),
                    binaryPath("/osx/googlechrome/32bit/2.10/chromedriver"));
            put(binaryFor(CHROME, LINUX, ARCHITECTURE_32_BIT),
                    binaryPath("/linux/googlechrome/32bit/2.10/chromedriver"));
            put(binaryFor(DriverType.PHANTOMJS, WINDOWS, ARCHITECTURE_64_BIT),
                    binaryPath("/windows/phantomjs/64bit/1.9.2/phantomjs.exe"));
            put(binaryFor(DriverType.PHANTOMJS, MAC, ARCHITECTURE_64_BIT),
                    binaryPath("/osx/phantomjs/64bit/1.9.7/phantomjs"));
            put(binaryFor(DriverType.PHANTOMJS, LINUX, ARCHITECTURE_64_BIT),
                    binaryPath("/linux/phantomjs/64bit/1.9.7/phantomjs"));
            put(binaryFor(DriverType.PHANTOMJS, WINDOWS, ARCHITECTURE_32_BIT),
                    binaryPath("/windows/phantomjs/32bit/1.9.7/phantomjs.exe"));
            put(binaryFor(DriverType.PHANTOMJS, MAC, ARCHITECTURE_32_BIT),
                    binaryPath("/osx/phantomjs/32bit/1.9.7/phantomjs"));
            put(binaryFor(DriverType.PHANTOMJS, LINUX, ARCHITECTURE_32_BIT),
                    binaryPath("/linux/phantomjs/32bit/1.9.7/phantomjs"));
            put(binaryFor(IE, WINDOWS, ARCHITECTURE_64_BIT),
                    binaryPath("/windows/internetexplorer/64bit/2.42.0/IEDriverServer.exe"));
            put(binaryFor(IE, WINDOWS, ARCHITECTURE_32_BIT),
                    binaryPath("/windows/internetexplorer/32bit/2.42.0/IEDriverServer.exe"));
        ...
        if (!useRemoteWebDriver) {
            ...
            if (null != binarySystemProperty
                ...
                System.out.println("Setting System Property '"
                        + binarySystemProperty + "'='" + binaryConfiguration
                        + "'");
