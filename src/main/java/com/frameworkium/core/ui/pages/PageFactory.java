...
public class PageFactory {
    ...
    public static <T extends BasePage<T>> T newInstance(Class<T> clazz) {
        return Guice.createInjector(new WebDriverGuiceModule()).getInstance(clazz).get();
    }

    public static <T extends BasePage<T>> T newInstance(Class<T> clazz, Integer timeoutInSeconds) {
        return Guice.createInjector(new WebDriverGuiceModule()).getInstance(clazz).get(timeoutInSeconds);
    }
