...
import static com.frameworkium.ui.config.DriverSetup.useRemoteDriver;
...
public abstract class DriverType {
    ...
    public Proxy getProxy() {
        if (SystemProperty.PROXY.isSpecified()) {
            Proxy proxy = new Proxy();
            String proxyString = SystemProperty.PROXY.getValue().toLowerCase();
            switch (proxyString) {
    ...
    public void maximiseBrowserWindow() {
        if (!SystemProperty.MAXIMISE.isSpecified() || Boolean.parseBoolean(SystemProperty.MAXIMISE.getValue())) {
            if (!useRemoteDriver() && !isNative() || SystemProperty.GRID_URL.isSpecified()) {
    ...
    public static boolean isNative() {
        return SystemProperty.APP_PATH.isSpecified();
    }
